...
export const VideoLayout: React.FC<VideoLayoutProps> = ({
  ...
  // Handle initial video playback when session starts
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (initialVideo) {
      initialVideo.addEventListener('ended', handleInitialVideoEnd);
    }

    return () => {
      if (initialVideo) {
        initialVideo.removeEventListener('ended', handleInitialVideoEnd);
      }
    };
  }, []);
  ...
  return (
    ...
    >
      ...
      <video
        ...
        style={{
          opacity: showInitialVideo ? 1 : 0,
          visibility: showInitialVideo ? 'visible' : 'hidden',
      ...
      <video
        ...
        className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-auto h-auto max-w-full max-h-full object-contain transition-opacity duration-300"
        style={{
          opacity: shouldShowStream || showInitialVideo ? 0 : 1,
          visibility: shouldShowStream || showInitialVideo ? 'hidden' : 'visible',
          display: isChromaKeyEnabled ? 'none' : 'block'
        }}
        loop
        muted
