...
export function applyChromaKey(
  ...
  const scaleFactor = options.scaleFactor || 0.25; // Reduced default scale factor for better performance
  const skipPixels = options.skipPixels || 2; // Increased default pixel skip for better performance
  ...
  // Optimized green screen detection
  const threshold = options.threshold || 1.02;
  const greenThreshold = 10;
  ...
  for (let i = 0; i < length; i += 4 * skipPixels) {
    ...
    // Simplified green screen detection
    if (g > r * threshold && g > b * threshold && g > greenThreshold) {
...
export function setupChromaKey(
  ...
  let lastFrameTime = 0;
  let skipCounter = 0;
  const targetFrameTime = 1000 / 15; // Reduced to 15 FPS for better performance
  const maxFrameProcessingTime = 50; // Increased threshold for slow frames
  ...
  const render = (timestamp: number) => {
    ...
    try {
      // Adaptive frame skipping with more aggressive skipping
      const averageProcessingTime = processingTimes.length > 0 
        ? processingTimes.reduce((a, b) => a + b, 0) / processingTimes.length 
        : 0;
      
      const shouldSkip = frameTimeDelta < targetFrameTime || 
                        (averageProcessingTime > maxFrameProcessingTime && skipCounter < 3) ||
      ...
      if (!shouldSkip) {
        ...
        applyChromaKey(sourceVideo, targetCanvas, {
          ...options,
          scaleFactor: options.scaleFactor || 0.25,
          skipPixels: options.skipPixels || 2
        });
        
        // Measure and store processing time
        const processingTime = performance.now() - startTime;
        processingTimes.push(processingTime);
        if (processingTimes.length > 3) processingTimes.shift(); // Reduced history size
        ...
        if (processingTime > maxFrameProcessingTime && 
            timestamp - lastPerformanceWarning > 10000) { // Increased warning interval
  ...
  return () => {
    ...
    if ((globalThis as any).__chromaKeyOffscreenCanvas) {
      delete (globalThis as any).__chromaKeyOffscreenCanvas;
    }
  };
} 