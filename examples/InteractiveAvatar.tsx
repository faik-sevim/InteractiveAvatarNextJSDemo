...
function InteractiveAvatar() {
  ...
  const [isLoopVideoPlaying, setIsLoopVideoPlaying] = useState(true);
  ...
  // Function to properly close all active sessions
  const closeAllSessions = async () => {
    try {
      console.log('Closing all active sessions...');
      
      // First mute audio and disable microphone
      muteInputAudio();
      setMicrophoneEnabled(false);
      
      // Then stop the avatar session
      try {
        await stopAvatar();
      } catch (error: any) {
        // Ignore LiveKit DataChannel errors as they are expected during cleanup
      ...
      console.error('Error in closeAllSessions:', error);
    }
  };
  ...
  // Avatar talking state management
  useEffect(() => {
    const handleAvatarStartTalking = () => {
      console.log('Avatar started talking - disabling microphone');
    ...
    const handleAvatarStopTalking = () => {
      console.log('Avatar stopped talking - enabling microphone');
      setHasStartedTalking(false);
      setIsLoopVideoPlaying(true);
  ...
  // Ensure microphone state is always in sync with avatar talking state
  useEffect(() => {
    if (hasStartedTalking) {
      console.log('Avatar is talking - ensuring microphone is disabled');
      setMicrophoneEnabled(false);
      muteInputAudio();
    } else {
      console.log('Avatar is not talking - ensuring microphone is enabled');
      setMicrophoneEnabled(true);
      unmuteInputAudio();
    }
  }, [hasStartedTalking, muteInputAudio, unmuteInputAudio]);
